...
exports a flat arm-observables layer for downstream robotics code. Delta adds
an instance-aware local Grounded-SAM-2 object layer that produces stable class
prompts, instance-scoped masks, tracks, interaction candidates, and review
overlays for generic tabletop stacking scenes.

Epsilon, Zeta, Eta, and Theta now exist as **experimental MVP back-half
phases**. They preserve honest provenance, emit compile-validated MuJoCo asset
packages, export a measured-world arm+gripper replay trace, and compile a
canonical robot-in-scene MuJoCo task package with fixed-camera playback
renders, but they do **not** yet claim research-grade dense static
reconstruction, learning, or deployment.
...
- Tests covering the supported contract, validation rules, and the current beta-through-theta MVP logic
...
vtc eta retarget-monocular --spec-dir spec --beta-dir ./artifacts/run01 --gamma-dir ./artifacts/run01 --delta-dir ./artifacts/run01 --epsilon-dir ./artifacts/run01 --zeta-dir ./artifacts/run01 --out-dir ./artifacts/run01
...
- `task.yaml` to declare scene instances and task goals on top of ontology IDs instead of relying on free-form naming
...
- `camera/robot_base_in_metric_world.json`
- `scene/sparse_points.ply`
- `scene/static_geometry_subset.json`
...
The monocular delta pipeline writes instance-aware stacking artifacts:
...
The current epsilon MVP writes dense-static-plus-cuboid scene artifacts while
preserving truthful provenance:
...
These paths are stable for later phases, and the checked-in profile now marks
them as `dense_static_reconstruction` / `inherited_from_beta`.
...
Zeta currently exports geometry-backed visual meshes with simpler collision
proxies, and it requires a successful MuJoCo compile and smoke test to claim
...
Eta is intentionally narrow in the current profile: UR5e + 2F-85, measured
world-frame task-window replay, and the `pinocchio_seed` IK backend contract.
